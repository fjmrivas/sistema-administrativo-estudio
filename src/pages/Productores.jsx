...
export default function Productores() {
  ...
  if (!empresaId) {
    return (
      <div>
        <h2 className="mb-6 text-xl font-semibold text-navy">Productores</h2>
  ...
  async function borrar(id) {
    if (!window.confirm('¿Eliminar este productor? Esta acción no se puede deshacer.')) return

    const { error: err } = await supabase.from('productores').delete().eq('id', id)
    if (err) {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-xl font-semibold text-navy">Productores</h2>
        <NuevoButton to="/productores/nuevo">+ Nuevo Productor</NuevoButton>
      </div>
